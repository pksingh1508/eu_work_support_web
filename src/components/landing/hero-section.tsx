...
export function HeroSection() {
  ...
  const backPhoneY = useTransform(scrollYProgress, [0, 1], [0, reduceMotion ? 0 : 105]);
  const frontPhoneY = useTransform(scrollYProgress, [0, 1], [0, reduceMotion ? 0 : 48]);
  const heroTextY = useTransform(scrollYProgress, [0, 1], [0, reduceMotion ? 0 : -28]);
  const heroOpacity = useTransform(scrollYProgress, [0, 0.8], [1, 0.45]);

  return (
    <section ref={sectionRef} className="relative isolate overflow-hidden bg-[#fafdff]">
      ...
      <div className="mx-auto grid min-h-[calc(100svh-78px)] max-w-[1240px] items-center gap-12 px-5 pb-16 pt-12 sm:px-8 sm:pb-20 sm:pt-16 lg:grid-cols-[1.04fr_0.96fr] lg:gap-8 lg:py-20">
        <m.div style={{ y: heroTextY, opacity: heroOpacity }} className="relative z-10 min-w-0">
          <m.div
          ...
          >
            Work across Europe with <span className="text-gradient">clarity.</span>
          </m.h1>
          <m.p
          ...
          >
            {appName} brings country guides, document checklists, saved resources,
            and practical support into one simple app—so your next move feels manageable.
          </m.p>

          <m.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.26 }}>
          ...
          <m.dl
            initial="hidden"
            animate="visible"
            variants={{ visible: { transition: { staggerChildren: 0.07, delayChildren: 0.36 } } }}
            className="mt-9 flex flex-wrap gap-x-6 gap-y-3 text-sm text-slate-500"
          >
            {[["One-time", "PRO payment"], ["Lifetime", "app access"], ["Secure", "Stripe checkout"]].map(([value, label]) => (
              <m.div key={value} variants={{ hidden: { opacity: 0, y: 10 }, visible: { opacity: 1, y: 0 } }} className="flex items-center gap-2">
                <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[#dff8ef] text-[11px] font-black text-[#138466]">✓</span>
        ...
        <div className="relative mx-auto h-[485px] w-full max-w-[540px] min-w-0 sm:h-[600px] lg:h-[650px]">
          <m.div
            ...
            transition={{ type: "spring", stiffness: 95, damping: 17, delay: 0.12 }}
          ...
          >
            <PhoneFrame src="/assets/home.jpg" alt="EU Work Support home screen" eager className="absolute left-1/2 top-[3%] w-[50%] -translate-x-1/2" />
          </m.div>
          <m.div
          ...
          >
            <div className="flex items-center gap-3">
              <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#dff8ef] text-[#138466]">✓</span>
              <span><b className="block text-xs text-[#101d36] sm:text-sm">Guides saved</b><span className="text-[11px] text-slate-500 sm:text-xs">Ready when you are</span></span>
            </div>
          </m.div>
        </div>
      </div>
